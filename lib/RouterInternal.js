...
class Router {
  static _changeHistory(url, push) {
    try {
      if (push) {
  ...
  static _findRouteByName(name) {
    return find(this._routes, ({ name: Name }) => name === Name);
  }

  static _findRouteByURL() {
    const { _routes } = this;
  ...
  static _init() {
    ...
    iterate(_routes, (route) => {
      ...
      const parent = find(_routes, ({ name }) => name === ParentName);
    ...
    window.addEventListener('click', (e) => {
      ...
      while (closestLink.tagName !== 'A') {
        closestLink = closestLink.parentNode;
      }

      if (closestLink && closestLink.getAttribute('target') !== '_blank' && !e.noRouting) {
        const push = !closestLink.hasAttribute('replace');

        e.preventDefault();

        this._forward(closestLink.getAttribute('href') || '', push);
      }
    }, false);
  }

  static _subscribe(name, callback) {
